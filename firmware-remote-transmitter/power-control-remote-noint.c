/**
@mainpage Power Control Project Remote Unit on AVR
@version 0.0.0
...
The protocol used needs to match the sync pattern of the RF receiver, but
otherwise the packets will have the same structure as that for the acquisition
project. The sync pattern is:
...
/* RFM02 Transmitter parameters specific for this application */
...
#define DEVIATION                    0
/* This bit rate is close to 2400 baud */
//#define BIT_RATE                  0x8F
/* This bit rate is close to 9600 baud */
#define BIT_RATE                  0x23
/* Output power dB (POR 0 = 0dB) */
#define POWER                        0
/* Carrier frequency to exactly 433 MHz (POR 7D0 = 435MHz
...
#define PM_SETTINGS_DEFAULT       0x00
...
{
#if defined __AVR_ATmega48__
/* Power down Watchdog Timer, comparator, USI, USART, Timers 0,1 */
    outb(PRR,0xFF);
#elif defined __AVR_ATtiny2313__
/* Power down analogue comparator and ports, and watchdog timer */
...
short delay. At 2400 bps the time to send the 7 bytes is 23 ms, so we get about
40 per second. Increasing bitrate to 9600 will change this by a factor of 4. */
...
/****************************************************************************
  Start Transmission of Data to the RFM02 over SDI
...
Issuing a sleep command will terminate data transmission.
...
uint8_t scan()
{
    ...
    {
/* Columns are active low, set current column to zero and remainder high */
...
/* Set four column pins as outputs set to zero */
    ...
    outb(COLUMNS,inb(COLUMNS) & ~COL_BITS);
/* Set port D2 for INT0 as input with pullup resistor activated */
    outb(INT_SETUP,inb(INT_SETUP) & ~_BV(INT_BIT));
    outb(INT_PORT,inb(INT_PORT) | _BV(INT_BIT));
/* Set lower three row pins as inputs with pullup resistors activated */
