...
/*@{*/
#define CONFIGURATION   RFM12_CMD_CONFIG | ((RF_BAND & 0x03) << 4) | \
                        ((CRYSTAL_LOAD_CAPACITANCE & 0x0F) << 0)
#define POWER           RFM12_CMD_POWER  | RX_EN | BBB_EN | TX_EN | SYN_EN | CRYSTAL_EN | \
                        LOW_BATT_EN | WAKE_TIMER_EN | CLOCK_OUT_DIS
#define FREQUENCY       RFM12_CMD_FREQ   | (CARRIER & 0xFFF)
#define RECEIVER        RFM12_CMD_RECV   | ((VDI_PIN & 0X01) << 10) | ((VDI_RESPONSE_TIME &0X03)<< 8) | \
                        ((BASEBAND_BW & 0x07)<< 5) | ((LNA_GAIN & 0x03) << 3) | \
                        ((RSSI_THRESHOLD & 0x07) << 0)
#define BATTERY         RFM12_CMD_BATT   | ((CLOCK_FREQ_OUT & 0x07) << 5) | (LBAT_LOWBATT & 0x1F)
#define AFC             RFM12_CMD_AFC    | ((AFC_MODE & 0x03) << 6) | ((AFC_RANGE & 0x03) << 4) | \
                        RFM01_AFC_FINE_ENABLE | RFM01_AFC_OUTPUT_ENABLE | RFM01_AFC_CALC_ENABLE | \
                        RFM01_AFC_FORCE_ENABLE
#define BASEFILTER      RFM12_CMD_BFILT  | ((DCR_ENABLE & 0x03) << 6) | ((DCR_TYPE & 0x03) << 3) | \
                        (DCR_DQDTHRESH & 0x07)
#define DATARATE        RFM12_CMD_RATE   | (BIT_RATE & 0x0FF)
#define FIFO            RFM01_CMD_FIFO   | ((FIFO_TRIGGER & 0x0F) << 4) | ((FIFO_START & 0x03) << 2) | \
                        (FIFO_ENABLE & 0x03)
#define WAKEUP          RFM12_CMD_WAKEUP | ((WAKEUP_EXPONENT & 0x1F) << 8) | (WAKEUP_DELAY & 0xFF)       
/*@}*/
